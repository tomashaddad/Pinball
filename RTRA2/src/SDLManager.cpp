...
bool SDLManager::initialise() {
    if (!initSDL()) throw GLException("Failed to initialise SDL", __FILE__, __LINE__);

    if (!initWindow()) throw GLException("Failed to create window", __FILE__, __LINE__);

    if (!initContext()) throw GLException("Failed to create OpenGL context", __FILE__, __LINE__);

    if (!initGLAD()) throw GLException("Failed to initialise GLAD", __FILE__, __LINE__);
...
bool SDLManager::initWindow() {
    if (m_window.fullscreen) {
        m_sdl.window = SDL_CreateWindow(m_window.title.c_str(), SDL_WINDOWPOS_UNDEFINED,
                                        SDL_WINDOWPOS_UNDEFINED, 0, 0,
                                        SDL_WINDOW_OPENGL | SDL_WINDOW_FULLSCREEN);
    } else {
        m_sdl.window = SDL_CreateWindow(
            m_window.title.c_str(), SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, m_window.width,
            m_window.height, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
    }
...
bool SDLManager::initGLAD() { return gladLoadGLLoader((GLADloadproc)SDL_GL_GetProcAddress); }

void SDLManager::swapBuffers() const { SDL_GL_SwapWindow(m_sdl.window); }
...
const unsigned int SDLManager::getWindowWidth() const { return m_window.width; }

const unsigned int SDLManager::getWindowHeight() const { return m_window.height; }
...
const unsigned int SDLManager::getPerformanceCounter() const { return SDL_GetPerformanceCounter(); }
